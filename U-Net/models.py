...
from tensorflow.keras.layers import Conv2DTranspose, Conv2D, Conv1D, BatchNormalization, \
    Activation, Input, UpSampling2D, MaxPooling2D, AveragePooling2D, MaxPooling1D, SpatialDropout2D, Lambda, Wrapper
...
def unet(input_shape, use_cnnt=False, nlayers=3):
    ...
    for ly in range(2, nlayers + 1):
        ...
        _tmp = unet_conv_block(_tmp, 2 * ly_outs[-1].shape[-1])
        ly_outs.append(_tmp)
        label2idx['box%d_out' % (ly)] = len(ly_outs) - 1

    # intermediate layers
    _tmp = MaxPooling2D(pool_size=2, strides=2, padding='same')(ly_outs[-1])
    _tmp = Conv2D(filters=ly_outs[-1].shape[-1], kernel_size=3, \
                  padding='same', activation='relu')(_tmp)
    ly_outs.append(_tmp)

    for ly, nch in zip(range(1, nlayers + 1), (128, 64, 32, 32)):  # (64, 32, 32)
        if use_cnnt:
            _tmp = Conv2DTranspose(filters=ly_outs[-1].shape[-1], activation='relu', \
    ...
    _tmp = Conv2D(filters=16, kernel_size=1, padding='same', activation='relu')(ly_outs[-1])

    _tmp = Conv2D(filters=1, kernel_size=1, padding='same', activation=None)(_tmp)
    # _tmp = tf.keras.activations.tanh(_tmp)
